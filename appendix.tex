...
\para{Objective function.} In order to minimize the cost, as we analyzed in Section \ref{sec:overall-opt}, we need to utilize the maximum capacity of each {\egress} as much as possible. Thus the objective function is set to minimize the sum of unused capacity at each {\egress}.
